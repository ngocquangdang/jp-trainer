...
const toList = (data: unknown): VocabItem[] => {
  ...
  const bucket = Array.isArray(source)
    ? (source[0] as Record<string, VocabDetail>)
    : (source as Record<string, VocabDetail>);
  if (!bucket) return [];
  return Object.keys(bucket).map((k) => ({
    key: k,
    detail: bucket[k] as VocabDetail,
  }));
};
            ...
            <>
              <div className="flex items-center justify-center gap-4">
                <div className="flex items-baseline gap-3">
                  ...
                  {item.detail.kanji && (
                    <div className="text-3xl text-gray-700" aria-label="Kanji">
                      {item.detail.kanji}
                    </div>
                  )}
                </div>
