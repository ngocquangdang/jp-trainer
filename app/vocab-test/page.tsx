...
import vocabData from "../../src/data/VocabList_N5.json";
...
type VocabList = Record<string, VocabEntry>;
...
export default function VocabTestPage() {
  ...
  const [viewMode, setViewMode] = useState<"hiragana" | "kanji">("hiragana");
  ...
  // Transform vocab data
  useEffect(() => {
    const data = vocabData[0] as VocabList;
    const transformedWords: VocabItem[] = Object.entries(data).map(([hiragana, details]) => ({
      hiragana,
      kanji: details.kanji,
      eng: details.eng,
      vni: details.vni,
      example: details.example,
      example_vni: details.example_vni,
    }));
    
    setWords(transformedWords);
    if (transformedWords.length > 0) {
      const indices = transformedWords.map((_, i) => i);
  ...
  const handleSubmit = () => {
    ...
    // Check if input matches English or Vietnamese meaning
    const input = inputValue.trim().toLowerCase();
    const isCorrect = 
      input === currentWord.eng.toLowerCase() || 
      input === currentWord.vni.toLowerCase();
      ...
      <div className="w-full max-w-md bg-white rounded-xl shadow-sm border border-gray-200 p-6">
        ...
        >
          {[
            ...
            const isActive = viewMode === (tab.key as "hiragana" | "kanji");
            const base =
              "w-full rounded-lg border px-3 py-2 text-sm font-medium focus:outline-none focus:ring-2 focus:ring-offset-2";
            const active =
              "border-purple-600 bg-purple-600 text-white focus:ring-purple-500";
            const inactive =
              "border-gray-300 bg-white text-gray-700 hover:border-gray-400 focus:ring-gray-300";
            ...
            return (
              <button
                ...
                className={className}
                onClick={() => setViewMode(tab.key as "hiragana" | "kanji")}
              >
                {tab.label}
              </button>
            );
        ...
        <div className="flex items-center justify-center gap-4 mt-6">
          <div className="text-7xl font-semibold tracking-wide text-gray-900 select-none">
            {viewMode === "hiragana" ? currentWord.hiragana : currentWord.kanji}
          </div>
          <AudioButton 
            text={viewMode === "hiragana" ? currentWord.hiragana : currentWord.kanji} 
            size="lg" 
          />
        </div>
        ...
        <input
          ...
          type="text"
          inputMode="text"
          aria-label="Nhập nghĩa tiếng Anh hoặc tiếng Việt"
          placeholder="Nhập nghĩa (EN hoặc VI) và nhấn Enter"
          value={inputValue}
        ...
        {finished && (
          <div className="mt-6 text-center text-gray-700">
            ...
            <div className="mt-3 flex items-center justify-center gap-2">
              ...
              <a
                href="/"
                className="rounded-lg bg-gray-200 hover:bg-gray-300 text-gray-800 px-3 py-2 text-sm"
              >
                Quay lại
              </a>
            </div>
